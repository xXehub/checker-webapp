                                    ...
                                    <button
                                        ...
                                        @if ($trackingData['status'] == 200)
                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                class="icon icon-tabler icon-tabler-check me-2 text-success"
                                        ...
                                        @else
                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                class="icon icon-tabler icon-tabler-alert-triangle me-2 text-danger"
                                                width="24" height="24" viewBox="0 0 24 24" stroke-width="2"
                                                stroke="currentColor" fill="none" stroke-linecap="round"
                                                stroke-linejoin="round">
                                                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                                    ...
                                                    <div class="card-body">
                                                        <div class="datagrid">
                                                            <div class="datagrid-item">
                                                                ...
                                                                <div class="datagrid-content">
                                                                    <div class="input-icon">
                                                                        <input type="text"
                                                                            value="{{ $trackingData['result']['resi'] }}"
                                                                            class="form-control"
                                                                            placeholder="Nomor Resi" readonly>
                                                            ...
                                                            <div class="datagrid-item">
                                                                ...
                                                                <div class="datagrid-content">
                                                                    {{ ucfirst($trackingData['result']['courier']) }}
                                                                </div>
                                                            </div>
                                                            <div class="datagrid-item">
                                                                <div class="datagrid-title">Pengirim</div>
                                                                <div class="datagrid-content">
                                                                    {{ $trackingData['result']['origin']['name'] }}
                                                                </div>
                                                            </div>
                                                            <div class="datagrid-item">
                                                                <div class="datagrid-title">Alamat Pengirim</div>
                                                                <div class="datagrid-content">
                                                                    {{ $trackingData['result']['origin']['address'] }}
                                                                </div>
                                                            </div>
                                                            <div class="datagrid-item">
                                                                <div class="datagrid-title">Penerima</div>
                                                                <div class="datagrid-content">
                                                                    {{ $trackingData['result']['destination']['name'] }}
                                                                </div>
                                                            </div>
                                                            <div class="datagrid-item">
                                                                <div class="datagrid-title">Alamat Penerima</div>
                                                                <div class="datagrid-content">
                                                                    {{ $trackingData['result']['destination']['address'] }}
                                                                </div>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <ul
                                                            class="steps steps-vertical steps-green steps-counter my-4">
                                                            @php
                                                                $sortedHistory = $trackingData['result']['history'];
                                                                usort($sortedHistory, function ($a, $b) {
                                                                    return strtotime($a['time']) -
                                                                        strtotime($b['time']);
                                                                });

                                                                // nhehapus note []
                                                                foreach ($sortedHistory as $key => $item) {
                                                                    // Use regex to replace any text within square brackets (including the brackets) with empty string
                                                                    $sortedHistory[$key]['note'] = preg_replace(
                                                                        '/\s*\[[^\]]*\]\s*/',
                                                                        ' ',
                                                                        $item['note'],
                                                                    );
                                                                    $sortedHistory[$key]['note'] = trim(
                                                                        $sortedHistory[$key]['note'],
                                                                    );
                                                                }
                                                            ...
                                                            @foreach ($sortedHistory as $index => $history)
                                                                <li
                                                                    class="step-item {{ $index === $activeIndex ? 'active' : '' }}">
                                                                    <div class="step-content">
                                                                        <div class="h4 m-0">{{ $history['time'] }}
                                                                        </div>
                                                                        <div class="text-muted">{{ $history['note'] }}
                                                                        </div>
                                                                    </div>
                                                                </li>
                                                            @endforeach
                                                    ...
                                                    <table class="table table-vcenter card-table m-0">
                                                        <tbody>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td class="text-wrap">Periksa kembali nomor resi yang
                                                                    dimasukkan</td>
                                                            </tr>
                                                        </tbody>
                                                    </table>
